(function(){
  ...
  function extraPaymentButton(payment){
    ...
    return `<button class="btn small gold" onclick="payExtraPayment('${payment.id}')">Paga ora</button>`;
  }

  function extraPaymentCard(payment){
    return `<p><b>${money(payment.amount_cents)}</b><br>${esc(payment.description)}<br>${extraPaymentButton(payment)}</p>`;
  }

  window.payExtraPayment = async function(id){
    try {
      const result = await api('/api/client/extra-payments/'+id+'/pay', { method:'POST', body:'{}' });
      if(result.url) location.href = result.url;
      else alert('Link pagamento non disponibile.');
    } catch(error) {
      alert(error.message || 'Errore pagamento preventivo');
    }
  };
  ...
  window.applyClientPayDateV1 = function(){
    try {
      ...
      window.clientHome = clientHome = function(d){
        const latest=d.reports.slice(0,3),pending=d.payments.filter(p=>p.status==='pending'),paymentValid=Boolean(d.customer?.payment_valid),upcoming=paymentValid?d.properties.filter(p=>p.active).slice(0,5):[];
        const nextHtml = paymentValid ? (upcoming.length?upcoming.map(p=>`<p><b>${esc(p.name)}</b><br>Prossimo controllo: ${fmtDate(p.next_check_date)}</p>`).join(''):'<p>Nessun controllo pianificato.</p>') : '<p class="muted">I controlli sono sospesi finché il pagamento non viene riattivato.</p>';
        document.getElementById('main').innerHTML=`<div class="grid"><div class="card"><h3>Stato</h3><span class="badge ${paymentValid?'ok':'bad'}">${paymentValid?'Pagamento regolare':'Pagamento non regolare'}</span><p>Piano cliente: <b>${P[normalizePlan(d.customer?.current_package_type)]||'Non ancora attivo'}</b></p></div><div class="card"><h3>Prossime scadenze</h3>${nextHtml}</div><div class="card"><h3>Preventivi / extra da pagare</h3>${pending.length?pending.map(extraPaymentCard).join(''):'<p>Nessun pagamento extra in sospeso.</p>'}</div></div>${planPaymentBox(d)}<div class="card" style="margin-top:14px"><h2>Ultimi report</h2>${latest.length?latest.map(reportHtml).join(''):'<p>Nessun report disponibile.</p>'}</div>`;
        setupRenewEvents();
        formatVisibleDates();
      };

      window.clientPayments = clientPayments = function(d){
        const rows = d.payments.length ? `<table><thead><tr><th>Descrizione</th><th>Importo</th><th>Stato</th><th></th></tr></thead><tbody>${d.payments.map(p=>`<tr><td>${esc(p.description)}</td><td>${money(p.amount_cents)}</td><td><span class="badge ${p.status==='paid'?'ok':p.status==='canceled'?'bad':'warn'}">${p.status==='pending'?'Da pagare':p.status==='paid'?'Pagato':p.status}</span></td><td>${extraPaymentButton(p)}</td></tr>`).join('')}</tbody></table>` : '<p>Nessun preventivo o extra.</p>';
